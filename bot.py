...
gc = gspread.service_account(
    filename=r'C:\Users\alema\Downloads\ksu-esports-tournament-bot-main\ksu-esports-tournament-bot-main\credentials.json')
...
async def on_ready():
    ...
    session = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15))  # Increase timeout to 15 seconds
...
class lobby:
    def __init__(self):
        ...
        for person in players:
            person.pop(0)  # Remove the first column (if necessary)
            participantToAdd = person[:7]  # Extract relevant player data
            intermediateList.extend(participantToAdd)

        finalPlayerList = intermediateList[:70]  # Limit to 70 players (10 teams of 7 players each)
